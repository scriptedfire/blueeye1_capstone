...
def node_voltage_calculator(cond_mat: np.ndarray, ic_mat: np.ndarray) -> np.ndarray:
    """ This function calculates the nodal voltage matrix. It uses the conduction and current vector matrices to calculate
                       this values
                       @param: cond_mat: numpy array that contains the conductance matrix data
    ...
    # multiplying inverse of conductance matrix with current vector
...
def cond_mat_generator(nodal_index: dict, reverse_map: dict) -> np.ndarray:
    """ This function generates the conductance matrix for the given grid. It takes in the nodal index information to
                        generate a nxn matrix where n is the number of nodes in the system. The reverse_map dictionary
                        contains the resistance/conductance data and information regarding where each element in the grid
                        is connected.
                          ...
                          return: conductance_matrix: a numpy array of the conductance matrix for a grid. This matrix
                          is static.
                      """
            ...
            try:
                if len(value) == 1:
                    gics = float((value[0][1])) + float(
                        ((1 / value[0][0][2]) * (nodal_volt_mat[key_list2[1]] - nodal_volt_mat[key_list2[0]])))
