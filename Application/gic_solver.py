...
def get_line_Efield(df_3D: pd.DataFrame, from_coords: list, to_coords: list) -> pd.DataFrame:
    ...
    for i, time in enumerate(time_array):
        north_east_Efield = [df_3D.loc[(time, north_east[0], north_east[1]), 'Ex'],
                             df_3D.loc[(time, north_east[0], north_east[1]), 'Ey']]
        north_west_Efield = [df_3D.loc[(time, north_west[0], north_west[1]), 'Ex'],
                             df_3D.loc[(time, north_west[0], north_west[1]), 'Ey']]

        south_east_Efield = [df_3D.loc[(time, south_east[0], south_east[1]), 'Ex'],
                             df_3D.loc[(time, south_east[0], south_east[1]), 'Ey']]
        south_west_Efield = [df_3D.loc[(time, south_west[0], south_west[1]), 'Ex'],
                             df_3D.loc[(time, south_west[0], south_west[1]), 'Ey']]
